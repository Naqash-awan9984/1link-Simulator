...
# IBFT STAN idempotency marker (unique per day)
# Tracks STANs that have already been successfully processed today.
# A duplicate STAN (even with a new RRN) is rejected as DUPLICATE TRANSACTION.
# Structure: {stan: True}
STAN_MARKER = {}
STAN_MARKER_DATE = datetime.now().date()
STAN_MARKER_LOCK = threading.Lock()
...
# ---------------------------------------------------------------------------
# STAN idempotency marker helpers
# ---------------------------------------------------------------------------

def _reset_stan_marker_if_needed():
    """Reset the STAN marker store at the start of each new day."""
    global STAN_MARKER_DATE, STAN_MARKER
    today = datetime.now().date()
    if today != STAN_MARKER_DATE:
        with STAN_MARKER_LOCK:
            if today != STAN_MARKER_DATE:
                STAN_MARKER = {}
                STAN_MARKER_DATE = today
                log(f"STAN marker reset for new day: {today}")


def _check_and_mark_stan(stan_value):
    """
    Atomically check whether a STAN has already been processed today.
    Returns True (is a duplicate) if the STAN was already marked.
    Returns False (is new) and records the STAN if it is fresh.
    An empty/missing STAN is never treated as a duplicate.
    """
    stan = str(stan_value or "").strip()
    if not stan:
        return False
    _reset_stan_marker_if_needed()
    with STAN_MARKER_LOCK:
        if stan in STAN_MARKER:
            return True
        STAN_MARKER[stan] = True
        return False
    ...
    def do_POST(self):
        ...
        try:
            ...
            # ------------------------------------------------
                ...
                # STAN idempotency marker: reject if this STAN was already processed today,
                # even when the client retries with a fresh RRN.
                stan_value = str(request_json.get("STAN", "") or "")
                if _check_and_mark_stan(stan_value):
                    log(f"IBFT duplicate detected via STAN marker: STAN={stan_value}")
                    log("================================")
                    log(f"Endpoint: {self.path}")
                    log(f"Request: {request_json}")
                    self.send_response(200)
